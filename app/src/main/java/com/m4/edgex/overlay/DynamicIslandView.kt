...
class DynamicIslandView(context: Context) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val requiredTextWidth = (contentText.length * charWidth) - spacing * 2
        val horizontalPadding = 18f * density * islandScale // Robust padding
        ...
        if (expansionProgress > 0.8f && contentText.isNotEmpty()) {
            ...
            // Calculate required width based on text length
            val requiredTextWidth = (contentText.length * charWidth) - spacing * 2
            val horizontalPadding = 16f * density * islandScale // Increased padding
            
            DotMatrixVisualizer.drawText(
                canvas,
                contentText,
                centerX - requiredTextWidth / 2f,
                currentY + (currentHeight - 7 * (dotSize + spacing)) / 2f + (0.5f * density), // Centered vertically
                dotSize,
                spacing,
                contentPaint,
